...
public class RestTemplateInitRegionProcessorFixture {
    ...
    protected ApiAccessTokenResponse 유효한_액세스_토큰;
    protected ApiAccessTokenResponse 유효하지_않은_액세스_토큰;
    protected ResultApiRegionResponse 서울특별시;
    protected ResultApiRegionResponse 강남구;
    protected ResultApiRegionResponse 개포1동;
    protected TotalApiRegionResponse 첫번째_지역_목록_조회_응답;
    protected TotalApiRegionResponse 서울특별시_하위_지역_목록_조회_응답;
    protected TotalApiRegionResponse 서울특별시_강남구_하위_지역_목록_조회_응답;

    @BeforeEach
    void setUp() {
        유효한_액세스_토큰 = new ApiAccessTokenResponse(
                Collections.singletonMap("accessToken", 액세스_토큰)
        );
        유효하지_않은_액세스_토큰 = new ApiAccessTokenResponse(null);

        서울특별시 = new ResultApiRegionResponse("서울특별시", 서울특별시_지역_코드);
        강남구 = new ResultApiRegionResponse("강남구", 강남구_지역_코드);
        개포1동 = new ResultApiRegionResponse("개포1동", 개포1동_지역_코드);
        첫번째_지역_목록_조회_응답 = new TotalApiRegionResponse(List.of(서울특별시));
        서울특별시_하위_지역_목록_조회_응답 = new TotalApiRegionResponse(List.of(강남구));
        서울특별시_강남구_하위_지역_목록_조회_응답 = new TotalApiRegionResponse(List.of(개포1동));
    }
}
