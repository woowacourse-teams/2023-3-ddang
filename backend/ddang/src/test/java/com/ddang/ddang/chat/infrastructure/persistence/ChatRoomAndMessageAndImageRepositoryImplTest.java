...
import com.ddang.ddang.chat.infrastructure.persistence.fixture.ChatRoomAndMessageAndImageRepositoryImplFixture;
...
class ChatRoomAndMessageAndImageRepositoryImplTest extends ChatRoomAndMessageAndImageRepositoryImplFixture {

    ChatRoomAndMessageAndImageRepository chatRoomAndMessageAndImageRepository;

    @BeforeEach
    void setUp(@Autowired final JPAQueryFactory queryFactory) {
        chatRoomAndMessageAndImageRepository =
                new ChatRoomAndMessageAndImageRepositoryImpl(new QuerydslChatRoomAndMessageAndImageRepository(queryFactory));
    }
    ...
    void 지정한_사용자_아이디가_포함된_채팅방을_조회한다() {
        ...
        final List<ChatRoomAndMessageAndImageDto> actual =
                chatRoomAndMessageAndImageRepository.findAllChatRoomInfoByUserIdOrderByLastMessage(엔초.getId());
