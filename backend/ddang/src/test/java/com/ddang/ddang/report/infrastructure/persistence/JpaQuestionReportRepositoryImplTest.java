...
import com.ddang.ddang.report.infrastructure.persistence.fixture.JpaQuestionReportRepositoryImplFixture;
...
class JpaQuestionReportRepositoryImplTest extends JpaQuestionReportRepositoryImplFixture {
    ...
    void 신고한_질문이라면_참을_반환한다() {
        // when
        final boolean actual = questionReportRepository.existsByQuestionIdAndReporterId(이미_신고한_질문.getId(), 신고자.getId());
    ...
    void 신고_전인_질문이라면_거짓을_반환한다() {
        // when
        final boolean actual = questionReportRepository.existsByQuestionIdAndReporterId(질문.getId(), 신고자.getId());
